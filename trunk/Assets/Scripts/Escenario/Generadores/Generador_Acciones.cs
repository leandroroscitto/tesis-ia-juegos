   ...
   public void generar() {
	  ...
	  foreach (Jugador jugador in jugadores) {
		 foreach (Waypoint waypoint in Navigation.Waypoints) {
			acciones.Add(new Accion(i, Accion.TipoAccion.MOVIMIENTO, jugador, waypoint, waypoint));
			i++;
			foreach (Connection connection in waypoint.Connections) {
			   acciones.Add(new Accion(i, Accion.TipoAccion.MOVIMIENTO, jugador, connection.From, connection.To));
			   i++;
			}
		 }
	  }
